...
class DirKLdiv( ) :
    def __init__(self, var, *args ) :
        ...
        if self.choice not in [None, "uniform", "log-uniform", "scaled"] :
...
def optimal_dirichlet_param_( compExp ) :
    '''.'''
    def myfunc( var, *args ) :
        return - Posterior(var, *args).log()
    def myjac(var, *args) :
        return - Posterior(var, *args).log_jac()
    return myMinimizer( myfunc, [INIT_GUESS], (compExp,), jac=myjac )

def optimal_entropy_param_( compExp ) :
    '''.'''
    def myfunc(var, *args) :
        return - ( Posterior(var, *args).log() + DirEntr(var, args[0].K).logMetapr() )
    def myjac(var, *args) :
        return - ( Posterior(var, *args).log_jac() + DirEntr(var, args[0].K).logMetapr_jac() )
    return myMinimizer( myfunc, [INIT_GUESS], (compExp,), jac=myjac )

def optimal_crossentropy_param_( compExp ) :
    '''(obsolete)''' 
    def myfunc(var, *args) :
        return - ( Posterior(var, *args).log() + DirCrossEntr(var, args[0].K).logMetapr() )
    def myjac(var, *args) :
        return - ( Posterior(var, *args).log_jac() + DirCrossEntr(var, args[0].K).logMetapr_jac() )
    return myMinimizer( myfunc, [INIT_GUESS], (compExp,), jac=myjac )

def optimal_divergence_params_( compDiv, choice="log-uniform" ) :
    '''.'''
    def myfunc(var, *args) :
        LogLike = DirKLdiv(var, args[0].K, args[1]).logMetapr()
        LogLike += Posterior(var[0], args[0].compact_1).log() + DirEntr(var[0], args[0].K).logMetapr()
        LogLike += Posterior(var[1], args[0].compact_2).log() + DirCrossEntr(var[1], args[0].K).logMetapr()
        return - LogLike
    def myjac(var, *args) :
        jac_LogLike = DirKLdiv(var, args[0].K, args[1]).logMetapr_jac()
        jac_LogLike[:,0] += Posterior(var[0], args[0].compact_1).log_jac() + DirEntr(var[0], args[0].K).logMetapr_jac()
        jac_LogLike[:,1] += Posterior(var[1], args[0].compact_2).log_jac() + DirCrossEntr(var[1], args[0].K).logMetapr_jac()
        return - jac_LogLike
...
def log_meta_posterior_hess( var, *args ) :
    '''.'''
    hess_LogLike = DirKLdiv(var, args[0].K, args[1]).logMetapr_hess()
    hess_LogLike[:,0,0] += Posterior(var[0], args[0].compact_1).log_hess()
    hess_LogLike[:,0,0] += DirEntr(var[0], args[0].K).logMetapr_hess()
    hess_LogLike[:,1,1] += Posterior(var[1], args[0].compact_2).log_hess() 
    hess_LogLike[:,1,1] += DirCrossEntr(var[1], args[0].K).logMetapr_hess()
    return hess_LogLike
