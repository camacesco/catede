...
def myMinimizer( myfunc, var, args, ) :
...
def optimal_dirichlet_param_( compExp ) :
    ...
    def myfunc( var, *args ) :
        alpha = var
        compExp = args[0]
        return - log_alphaLikelihood( compExp, alpha )

    return myMinimizer( myfunc, [1.], (compExp,) )
...
def optimal_entropy_param_( compExp ) :
    ...
    def myfunc( var, *args ) :
        ...
        compExp = args[0]
        K = compExp.K
        metaprior = ( K * triGmm(K*alpha+1) - triGmm(alpha+1) ) / ( diGmm(K*alpha+1) - diGmm(alpha+1) )
        return - ( log_alphaLikelihood( compExp, alpha ) + np.log(metaprior) )
...
def optimal_crossentropy_param_( compExp ) :
    ...
    def myfunc( var, *args ) :
        ...
        compExp = args[0]
        K = compExp.K
        metaprior = - ( K * triGmm(K*beta) - triGmm(beta) ) / ( diGmm(K*beta) - diGmm(beta) )
        return - ( log_alphaLikelihood( compExp, beta ) + np.log(metaprior) )
...
def optimal_dirichlet_EP_param_( compDiv ) :
    ...
    def myfunc( var, *args ) :
        ...
        return - ( log_alphaLikelihood( compExp_1, alpha ) + log_alphaLikelihood( compExp_2, beta ) )

    return myMinimizer( myfunc, [1.], (compDiv,) )
...
def optimal_divergence_EP_param_( compDiv ) :
    ...
    def myfunc( var, *args ) :
        ...
        compExp_1, compExp_2, = args[0].compact_1,  args[0].compact_2
        K = args[0].K
        metaprior = ( 1. - 1. / K ) / alpha**2 
        return - ( log_alphaLikelihood( compExp_1, alpha ) + log_alphaLikelihood( compExp_2, beta ) + np.log(metaprior) )

    return myMinimizer( myfunc, [1.,1.], (compDiv,) )
