...
'''
    symmetric Kullback-Leibler Divergence Estimator
...
class _sDKL_auxfunc( one_dim_metapr ) :
    def __init__( self, K ) :
        '''Class for `a priori` expected symmetric DKL under symmetric Dirichlet prior.'''
        self.K = K 
        self._sign = -1
        self.cnst = (K - 1.) / K
    def apriori( self, a ) :
...
class DirSymmetricKLdiv( two_dim_metapr ) :
...
class dpm_Symmetric_KL( dpm_wrapper ) :
    ...
    def __init__( self, cpct_div, choice="log-uniform", **kwargs ) :
        ...
        self.dir_meta_obj = DirSymmetricKLdiv(cpct_div.K, choice=choice, **kwargs)
...
def main( cpct_div, error=False, n_bins=None, n_sigma=3,
    ...
    dpm_wrap = dpm_Symmetric_KL(cpct_div, choice=choice, scaling=scaling)
